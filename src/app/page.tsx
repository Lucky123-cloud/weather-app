...
// Helper function for date formatting with ordinal
const formatDateWithOrdinal = (date: Date | string | number) => {
  const parsedDate = typeof date === 'string' ? parseISO(date) : 
                    typeof date === 'number' ? fromUnixTime(date) : date;
  ...
  const { isLoading, error, data, refetch } = useQuery<WeatherData>(
    ...
    async () => {
      const { data } = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${place}&appid=7cad67181be8d1f01116b8bea7873679&cnt=56`
      );
      return data;
    }
  );
            ...
            <section className="space-y-4">
              <div className="space-y-2">
                <h2 className="flex gap-1 text-2xl items-end">
                  <p>{firstData?.dt && format(fromUnixTime(firstData.dt), "EEEE")}</p>
                  <p className="text-lg">
                    ({firstData?.dt && formatDateWithOrdinal(firstData.dt)})
                  </p>
                </h2>
            ...
            <section className="flex w-full flex-col gap-4">
              ...
              {firstDataForEachDate.map((d, i) => (
                <ForecastWeatherDetail
                  ...
                  date={d ? formatDateWithOrdinal(d.dt) : ""}
                  day={d ? format(fromUnixTime(d.dt), "EEEE") : ""}
